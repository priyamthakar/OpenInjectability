...
| D — independent experimental validation | **Pipeline ready; data still blocked** | `validate-experimental` + `validation/experimental/` shipped; multi-agent scrape + Allmendinger/Verwulgen hunt: **0** absolute fluid-only force rows; Verwulgen geometry setup archived; `validation_status` unchanged |
